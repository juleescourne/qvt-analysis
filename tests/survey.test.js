...
const source = readFileSync(resolve(__dirname, '../src/utils/survey.js'), 'utf8');
...
test('a clicked response selects the same respondent identities in every distribution',async()=>{
 const { filterRows, distribution } = await surveyModule;
...
test('question and department filters intersect; clearing restores the population',async()=>{
 const { filterRows } = await surveyModule;
...
test('invalid responses are excluded, decimal category means are rounded only for bins',async()=>{
 const { distribution, rating, questionColumns } = await surveyModule;
...
test('synthetic dataset has balanced departments, stable identity and aligned coordinates',async()=>{
 const csv=readFileSync(resolve(__dirname, '../public/examples/survey_sample.csv'),'utf8').trim().split(/\r?\n/).map(x=>x.split(','));
 const xy=readFileSync(resolve(__dirname, '../public/examples/projection_sample.csv'),'utf8').trim().split(/\r?\n/);
